...
namespace ZAJCZN.MIS.Helpers
{
    ...
    {
        #region - 检查目录是否存在，若不存在则创建目录 -

        /// <summary>
        /// 检查目录是否存在，若不存在则创建目录
        /// </summary>
        public static void CheckFolder(string directoryPath)
        {
        ...
        /// </summary>
        /// <param name="sourceFilePath">需要移动的源文件的绝对路径</param>
        /// <param name="moveToFilePath">移动后文件的绝对路径</param>
        public static void MoveFile(string sourceFilePath, string moveToFilePath)
        {
            string dicName = Path.GetDirectoryName(moveToFilePath);
            if (!IsExistDirectory(dicName))
                CreateDirectory(dicName);
            if (File.Exists(sourceFilePath))
            {
                File.Move(sourceFilePath, moveToFilePath);
            }
        }
