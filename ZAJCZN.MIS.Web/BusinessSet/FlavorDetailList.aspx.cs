...
{
    public partial class FlavorDetailList : PageBase
    {
        #region ViewPower
        ...
        public override string ViewPower
        {
            get
            {
                return "CoreFlavorView";
            }
        }

        #endregion

        private int FlavorTypeID
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (FlavorTypeID < 0)
                {
                    PageContext.Redirect("~/BusinessSet/FlavorManage.aspx");
                }
                LoadData();
            }
        }
        ...
        {
            // 权限检查
            CheckPowerWithButton("CoreFlavorEdit", btnChangeEnableUsers);
            CheckPowerWithButton("CoreFlavorEdit", btnNew);

            Inits();

            btnNew.OnClientClick = Window1.GetShowReference(string.Format("~/BusinessSet/FlavorDetailEdit.aspx?typeid={0}", FlavorTypeID), "新增口味备注");
        ...
        private void BindGrid()
        {
            ...
            {
                qryList.Add(Expression.Like("FlavorName", qryName, MatchMode.Anywhere));
            }
            if (ddlIsUsed.SelectedValue != "0")
            {
                qryList.Add(Expression.Eq("IsUsed", int.Parse(ddlIsUsed.SelectedValue)));
            }
            qryList.Add(Expression.Eq("ParentID", FlavorTypeID));

            Order[] orderList = new Order[1];
            Order orderli = new Order(Grid1.SortField, Grid1.SortDirection == "ASC" ? true : false);
            orderList[0] = orderli;
            int count = 0;
            IList<Tm_FlavorInfo> list = Core.Container.Instance.Resolve<IServiceFlavor>().GetPaged(qryList, orderList, Grid1.PageIndex, Grid1.PageSize, out count);
        ...
        {
            // 数据绑定之前，进行权限检查
            CheckPowerWithWindowField("CoreFlavorEdit", Grid1, "editField");
            CheckPowerWithLinkButtonField("CoreFlavorEdit", Grid1, "deleteField");
        }
        ...
        private void SetSelectedUsersEnableStatus(bool enabled)
        {
            ...
            {
                Tm_FlavorInfo entity = Core.Container.Instance.Resolve<IServiceFlavor>().GetEntity(ID);
                entity.IsUsed = int.Parse(isUsed);
                Core.Container.Instance.Resolve<IServiceFlavor>().Update(entity);
            }
            // 重新绑定表格
            BindGrid();
        }
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            {
                Core.Container.Instance.Resolve<IServiceFlavor>().Delete(ID);
                BindGrid();
            }
        }
