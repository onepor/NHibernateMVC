...
{
    public partial class MealperiodManager : PageBase
    {
        #region ViewPower
        ...
        {
            //权限检查
            CheckPowerWithButton("CoreMealperiodEdit", btnNew); 
            

            Inits();
            btnNew.OnClientClick = Window1.GetShowReference("~/BusinessSet/MealperiodEdit.aspx?action=add", "新增餐段");
        ...
        private void BindGrid()
        {
            ...
            {
                qryList.Add(Expression.Like("MealsName", qryName, MatchMode.Anywhere));
            }
            Order[] orderList = new Order[1];
            Order orderli = new Order(Grid1.SortField, Grid1.SortDirection == "ASC" ? true : false);
            orderList[0] = orderli;
            int count = 0;
            IList<tm_Mealtime> list = Core.Container.Instance.Resolve<IServiceMealtime>().GetPaged(qryList, orderList, Grid1.PageIndex, Grid1.PageSize, out count);
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            {
                Core.Container.Instance.Resolve<IServiceMealtime>().Delete(ID);
                BindGrid();
            }
        }
