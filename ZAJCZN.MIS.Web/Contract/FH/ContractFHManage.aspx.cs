...
{
    public partial class ContractFHManage : PageBase
    {
        #region ViewPower
        ...
        {
            // 权限检查 
            CheckPowerWithButton("CoreFHOrderEdit", btnNew);
            //设置记录分页记录数
        ...
        {
            // 数据绑定之前，进行权限检查
            CheckPowerWithWindowField("CoreFHOrderEdit", Grid1, "editField");
            CheckPowerWithLinkButtonField("CoreFHOrderDelete", Grid1, "deleteField");
        }
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            if (orderInfo != null)
            {
                if (orderInfo.IsTemp == 1)
                {
                    ...
                    {
                        PageContext.Redirect(string.Format("~/Contract/FH/ContractFHEdit.aspx?id={0}", ID));
                    }
                }
                else
                {
        ...
        protected void btnNew_Click(object sender, EventArgs e)
        {
            ...
            //判读是否有执行中的合同，如没有，不能发货
            if (list.Count > 0)
            {
                PageContext.Redirect("~/Contract/FH/ContractFHEdit.aspx");
            }
            else
            {
                Alert.Show("当前没有执行中的合同，不能创建发货单！", MessageBoxIcon.Warning);
            }
        }
