...
{
    public partial class EquipmentTypeManage : PageBase
    {
        #region ViewPower
        ...
        {
            // 权限检查
            CheckPowerWithButton("CoreEquipmentTypeNew", btnNew);
            CheckPowerWithButton("CoreEquipmentTypeEdit", btnChangeEnableUsers);
            btnNew.OnClientClick = Window1.GetShowReference("~/Equipment/EquipmentTypeEdit.aspx", "新增商品类别");
            Inits();
            //绑定数据
            BindGrid();
        }
        ...
        {
            // 数据绑定之前，进行权限检查
            CheckPowerWithWindowField("CoreEquipmentTypeEdit", Grid1, "editField");
            CheckPowerWithLinkButtonField("CoreEquipmentTypeEdit", Grid1, "deleteField");
        }
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            {
                // 在操作之前进行权限检查
                if (!CheckPower("CoreEquipmentTypeEdit"))
                {
                    CheckPowerFailWithAlert();
                    return;
                }
                //删除类型
                Core.Container.Instance.Resolve<IServiceEquipmentTypeInfo>().Delete(ID);
                //更新页面数据
                BindGrid();
            }          
        }
        ...
        {
            int ID = GetSelectedDataKeyID(Grid1);
            PageContext.RegisterStartupScript(Window1.GetShowReference(string.Format("~/Equipment/EquipmentTypeEdit.aspx?id={0}", ID)
                , "编辑商品类别"));
