...
{
    public partial class ReceiveOrderEdit : PageBase
    {
        #region ViewPower
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                btnReturn.ConfirmText = "确定要取消当前领用单信息吗？";

                btnNew.OnClientClick = Window1.GetShowReference(string.Format("~/PublicWebForm/OrderReceiveSelectDialog.aspx?rowid={0}", OrderNO), "添加领用商品");
                // 删除选中单元格的客户端脚本 
        ...
        private void BindGrid()
        {
            ...
            orderList[0] = orderli;
            int count = 0;
            IList<ReceiveOrderDetail> list = Core.Container.Instance.Resolve<IServiceReceiveOrderDetail>().GetPaged(qryList, orderList, Grid1.PageIndex, Grid1.PageSize, out count);
        ...
        {
            CheckPowerWithLinkButtonField("CoreReceiveOrderEdit", Grid1, "deleteField");
        }
        ...
        protected void btnDeleteSelected_Click(object sender, EventArgs e)
        {
            ...
            {
                Core.Container.Instance.Resolve<IServiceReceiveOrderDetail>().Delete(id);
            }
            BindGrid();
        }
