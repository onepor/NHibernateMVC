...
namespace ZAJCZN.MIS.Web
{
    public partial class CarManager : PageBase
    {
        ...
        {
            //权限检查
            CheckPowerWithButton("CoreCarNew", btnNew);

            btnNew.OnClientClick = Window1.GetShowReference("~/SysSet/CarEdit.aspx?action=add", "新增送货车辆信息");
        ...
        public string GetCalcPeiceType(string state)
        {
            string i = "";
            switch (state)
            {
        ...
        public string GetChargingType(string state)
        {
            string i = "";
            switch (state)
            {
        ...
        {
            // 数据绑定之前，进行权限检查
            CheckPowerWithWindowField("CoreCarEdit", Grid1, "editField");
            CheckPowerWithLinkButtonField("CoreCarDelete", Grid1, "deleteField");
        }
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            {
                if (!CheckPower("CoreCarEdit"))
                    Alert.ShowInTop("您没有该操作权限，请从管理员处获取！", MessageBoxIcon.Information);
            }
            if (e.CommandName == "Delete")
            {
                if (!CheckPower("CoreCarDelete"))
                    Alert.ShowInTop("您没有该操作权限，请从管理员处获取！", MessageBoxIcon.Information);
                else
                {
                    Core.Container.Instance.Resolve<IServiceCarInfo>().Delete(ID);
                    BindGrid();
                }
            }
        }
