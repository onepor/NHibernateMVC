...
{
    public partial class CustomerManager : PageBase
    {
        #region ViewPower
        ...
        private void LoadData()
        {
            ...
            btnNew.OnClientClick = Window1.GetShowReference("~/SysSet/CustomerEdit.aspx?action=add", "新增客户信息");
        ...
        private void BindGrid()
        {
            ...
            orderList[0] = orderli;
            int count = 0;
            IList<CustomerInfo> list = Core.Container.Instance.Resolve<IServiceCustomerInfo>().GetPaged(qryList, orderList, Grid1.PageIndex, Grid1.PageSize, out count);
        ...
        {
            // 数据绑定之前，进行权限检查
            CheckPowerWithWindowField("CoreCustomerEdit", Grid1, "editField");
        ...
        { 
            ...
            {
                if (!CheckPower("CoreCustomerEdit"))
            ...
            if (e.CommandName == "Delete")
            {
                ...
                {
                    Core.Container.Instance.Resolve<IServiceCustomerInfo>().Delete(ID);
                    BindGrid();
                }
            }
        }
        ...
        {
            int ID = GetSelectedDataKeyID(Grid1);
            PageContext.RegisterStartupScript(Window1.GetShowReference(string.Format("~/SysSet/CustomerEdit.aspx?id={0}&action=edit", ID)
                , "编辑客户信息"));
