...
namespace ZAJCZN.MIS.Web.admin
{
    public partial class power : PageBase
    {
        ...
        private void BindGrid()
        {
            ...
            {
                qryList.Add(Expression.Like("Name", searchText, MatchMode.Anywhere) || Expression.Like("Title", searchText, MatchMode.Anywhere));
            }
            Order[] orderList = new Order[1];
            Order orderli = new Order(Grid1.SortField, Grid1.SortDirection == "ASC" ? true : false);
            orderList[0] = orderli;
            int count = 0;
            IList<powers> powerList = Core.Container.Instance.Resolve<IServicePowers>().GetPaged(qryList, orderList, Grid1.PageIndex, Grid1.PageSize, out count);
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Delete")
            {
                ...
                IList<ICriterion> qryList = new List<ICriterion>();
                qryList.Add(Expression.Eq("PowerID", powerID));
                int roleCount = Core.Container.Instance.Resolve<IServiceRolePowers>().GetRecordCountByFields(qryList);
                //DB.Roles.Where(r => r.Powers.Any(p => p.ID == powerID)).Count();
                if (roleCount > 0)
                {
                    Alert.ShowInTop("删除失败！需要先清空使用此权限的角色！");
                    return;
                }
                Core.Container.Instance.Resolve<IServicePowers>().Delete(powerID);
                // 执行数据库操作
